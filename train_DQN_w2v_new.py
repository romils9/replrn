...
class ActorLearner:
    ...
    def __init__(self, config, env, replay_buffer, dqn, target_dqn, last_frame):
        ...
        self.all_final_rewards = [] # will hold all episodic rewards
        self.all_final_episode_lengths = [] # will hold all episodic lengths
        ...
        self.optimizer = RMSprop(self.dqn.parameters(), lr=config['learning_rate'])
    ...
    def collect_experience(self):
        ...
        for _ in range(self.acting_learning_step_ratio):
            ...
            #manually tracking episode stats instead of depreciated env Monitor 
            ...
            if done_flag:
                new_frame , _ = self.env.reset()

                self.all_final_rewards.append(
                    self.current_episode_rewards
                    )
                
                self.all_final_episode_lengths.append(
    ...
    def sample_action(self, state):
        ...
        if self.get_number_of_env_steps() < self.num_warmup_steps:
    ...
    def get_episode_rewards(self):
        return self.current_episode_rewards
    ...
    def maybe_log_episode(self):
        ...
        rewards = self.all_final_rewards
        episode_lengths = self.all_final_episode_lengths
    ...
    def maybe_log(self):
        ...
        num_steps = self.get_number_of_env_steps()
...
def train_dqn(config):
    ...
    #env.reset returns a list of two items, we require the first item
    
    initial_state,_ = env.reset()

    actor_learner = ActorLearner(config, env, replay_buffer, dqn, target_dqn, initial_state)
